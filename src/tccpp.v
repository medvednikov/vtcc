...
fn cstr_vprintf(cstr &CString, msg string) int {
	...
	unsafe { C.memcpy(&u8(cstr.data) + cstr.size, msg.str, len) }
	cstr.size += len
	(&u8(cstr.data))[cstr.size] = 0
	return len
}
...
const target_os_defs = $if macos {
	['__APPLE__', '__APPLE_CC__', '__MACH__', '__LITTLE_ENDIAN__', '__unix__', '__unix']
} $else {
...
fn tcc_predefs(s1 &TCCState, cs &CString, is_asm int) {
	...
	cstr_cat(cs, c'#define __GNUC__ 4\n#define __GNUC_MINOR__ 2\n#define __GNUC_PATCHLEVEL__ 1\n', -1)
